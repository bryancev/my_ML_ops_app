    ...
    def upload():
        if request.method == 'POST':
            ...
            if file and allowed_file(file.filename):
                ...
                submission, probabilities = scorer.make_pred(preprocessed_df, save_location)
                ...
                #top_n feature importances
                top_features = scorer.get_feature_importances(top_n=5)
                with open(output_filename.replace('.csv', '_features.json'), 'w') as f:
                    json.dump(top_features, f, ensure_ascii=False)
                
                #prediction distribution plot
                scorer.plot_density_prediction_distribution(probabilities, output_filename.replace('.csv', '_distribution.jpg'))
